...
class Z3Encoder(GenericVisitor):
    ...
    def _get_constraint_var_param(self, node: Node, i: int, j: int):
        node_id = self._indexer.get_id(node)
        var_name = '@n{}_c{}_i{}'.format(node_id, i, j)
        return var_name
    ...
    def encode_param_alignment(self, node: Node, ty: ValueType, index: int):
        ...
        for pname, pty in ty.properties:
            ...
            cname = self._get_constraint_var_param(node, index, count)
            self._solver.assert_and_track(actual == expected, cname)
            if not isinstance(node, ApplyNode): self._unsat_map_param[cname] = (node, expected_expr)
            count += 1
    ...
    def get_blame_nodes(self):
        ...
        for v in unsat_core:
            if str(v) in self._unsat_map:
                node, cidx = self._unsat_map[str(v)]
                unsat_dict[node].append(node.production.constraints[cidx])
            elif str(v) in self._unsat_map_param:
                node, cstr = self._unsat_map_param[str(v)]
                unsat_dict[node].append(cstr)
...
class SmtBasicDecider(ExampleDecider):
    _imply_map: ImplyMap
    ...
    def __init__(self,
        ...
        self.output = None
        SmtBasicDecider._strong_imply_map = self._build_strong_imply_map(spec)
    ...
    def _build_strong_imply_map(self, spec: TyrellSpec):
        ...
        for prod0, prod1 in permutations(constrained_prods, r=2):
            lst = prod0.constraints
            res0 = lst[0]
            for i in range(1, len(lst)):
                res0 = BinaryExpr(BinaryOperator.AND, res0, lst[i])

            lst = prod1.constraints
            res1 = lst[0]
            for i in range(1, len(lst)):
                res1 = BinaryExpr(BinaryOperator.AND, res1, lst[i])

            if self._check_implies(res1, res0):
