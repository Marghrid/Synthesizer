...
class Optimizer:
    ...
    def __init__(self, solver, max_prod, variables, nodes):
        self.bound = 0
        self.ub = 0
        self.solver = solver
    ...
    def createVariablesOccurrence(self):
        for x in range(0, self.max_prod):
            name = 'occ' + str(x)
            v = Int(name)
    ...
    def mk_is_parent_subtree(self, child_prod, parent_prods):
        i = 0
        self.solver.add(self.variables[0] != child_prod.id)
        for x in range(1, len(self.nodes)):
            # FIXME: improve empty integration
            ctr = None
            for parent in parent_prods:
                if ctr is None:
                    ctr = Implies(self.variables[x] == child_prod.id, self.variables[self.nodes[x].parent.id - 1] == parent.id)
                else:
    ...
    def mk_is_parent(self, parent, child, weight=None):
        ...
        child_pos = []
        # find positions that type-check between parent and child
        for x in range(0, len(parent.prod.rhs)):
            if child.prod.lhs == parent.prod.rhs[x]:
                child_pos.append(x)
